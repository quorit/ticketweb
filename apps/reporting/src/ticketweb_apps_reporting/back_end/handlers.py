    ...
    def __init__(self):
        ...
        def get_ticket_content(real_name,req_content):
            ...
            if source_choice["rpt_source_type"] == "standard":
                report_txt = _get_shared_data()["data_support_choices"][source_choice["source_key"]]
            else:
                    ...
                    def build_common_faculty(common_progs,faculty):
                        def build_prog(progs,prog):
                            def build_subprogs(subprogs):
                                ...
                                return result              
                            subprogs = progs[prog]
                            result = _get_shared_data()["faculties_student"][faculty]["progs"][prog]["longhand"] \
            ...
            def build_progs(progs_selected):
                def build_first_year(progplans):
                    def build_faculty(faculty):
                        ...
                        result = faculty + ":<ul>" \
                                  + "".join(["<li>" + _get_shared_data()["faculties"][faculty][prog] + "</li>" for prog in progs]) \
                                  + "</ul>"
                        return result
