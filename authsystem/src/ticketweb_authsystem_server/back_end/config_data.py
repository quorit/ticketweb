...
_etc_path = os.getenv("CONFIG_DIR", "/etc/ticketweb/authsystem")




def _get_config_data():
    config_file = os.path.join(_etc_path,"config.json5")
...
def _get_applications():
    apps_file = os.path.join(_etc_path,"applications.json")
    f = open(apps_file,"r")
...
def _init_rsa_key_data():
    rsa_path = os.path.join(_etc_path,"rsa")
